    ...
    if len(usuarios) > 0:
        ...
        if fechaAhora == ultimaFecha:
            ...
            nuevaListaExperiencia = [
                listaExperiencia[0] + 20 if listaTareas[0] == 1 else listaExperiencia[0],
                listaExperiencia[1] + 20 if listaTareas[1] == 1 else listaExperiencia[1],
                listaExperiencia[2] + 20 if listaTareas[2] == 1 else listaExperiencia[2],
                listaExperiencia[3] + 20 if listaTareas[2] == 1 else listaExperiencia[3],
                listaExperiencia[4] + 20 if listaTareas[3] == 1 else listaExperiencia[4]
            ]
            ...
            newCursor.execute(""" UPDATE `Experiencia` SET Alimentacion='{}' ,Transporte='{}' ,Reciclaje='{}' ,Consumo='{}' ,Recursos='{}' WHERE `Experiencia`.id_usuario = '{}' """.format(listaExperienciaLevelup[0],listaExperienciaLevelup[1], listaExperienciaLevelup[2], listaExperienciaLevelup[3], listaExperienciaLevelup[4], session['id']))
            conn.commit()
            ...
            newCursor.execute(""" UPDATE `Tareas` SET '{}'='{}' ,'{}'='{}' ,'{}'='{}' ,'{}'='{}' WHERE `Tareas`.id_usuario = '{}' """.format('tarea 1', 0, 'tarea 2', 0, 'tarea 3', 0, 'tarea 4', 0 ,session['id']))
            conn.commit()

            newCursor.execute(""" UPDATE `Usuarios Registrados` SET 'ultimo_dia='{}'  WHERE `Experiencia`.id_usuario = '{}' """.format(fechaAhora ,session['id']))
            conn.commit()
...
def profile():
    if 'active' in session:
        ...
        #Cambiar valor o Eliminar sesion cuando se sale de la página -----------------------------------------------------
        #now = datetime.datetime.now()
        #fechaHoy = str(now.year) + '-' + str(now.month) + '-' + str(now.day)

        #newCursor.execute("""INSERT INTO `Usuarios Registrados` (id_usuario,nombre, contrasena, ultimo_dia) VALUES ('{}', '{}', '{}', '{}')""".format(session['id'], nombreUsuarioRegistrado, contrasenaUsuarioRegistrado, fechaHoy))
        #conn.commit()

        newCursor.close()
        return render_template(f'nivel-{str(PromedioNivel)}.html',
            nombre=nombre,
